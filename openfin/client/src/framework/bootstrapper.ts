...
import { deregister as deregisterDock, minimize as minimizeDock, register as registerDock, show as showDock } from './workspace/dock';
import { deregister as deregisterHome, hide as hideHome, register as registerHome, show as showHome } from './workspace/home';
import { deregister as deregisterNotifications, register as registerNotifications, show as showNotifications, hide as hideNotifications } from './workspace/notifications';
...
export async function init(): Promise<boolean> {
  ...
  bootstrapOptions.autoShow = bootstrapOptions.autoShow.filter((component) => registeredComponents.includes(component) || component === 'none');
  ...
  if (bootstrapOptions.autoShow.length === 0 && registeredComponents.length > 0) {
    logger.info(`No auto show options specified but at least one Workspace Component is registered. Showing the first registered component: ${registeredComponents[0]}`);
