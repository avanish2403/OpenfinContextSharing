...
import { closedownModule, closedownModules, initializeModule, initializeModules, loadModules } from './modules';
import { launchPage, launchView } from './platform/browser';
import type { IntegrationHelpers, IntegrationModule, IntegrationModuleDefinition, IntegrationProviderOptions } from './shapes/integrations-shapes';
...
let integrationModules: ModuleEntry<IntegrationModule, IntegrationHelpers, unknown, IntegrationModuleDefinition>[] = [];
 ...
 */
export async function init(options: IntegrationProviderOptions, helpers: ModuleHelpers, homeRegistration: HomeRegistration): Promise<void> {
  if (options) {
    ...
    integrationHelpers = {
      ...
      setSearchQuery: homeRegistration.setSearchQuery ? async (query) => homeRegistration.setSearchQuery(query) : undefined,
    ...
    // Load the modules
    integrationModules = await loadModules<IntegrationModule, IntegrationHelpers, unknown, IntegrationModuleDefinition>(options, 'integrations');
    ...
    for (const integration of integrationModules) {
      ...
      integration.definition.autoStart = integrationPreference?.autoStart ?? integration.definition.autoStart ?? true;
...
export async function getSearchResults(
  ...
  if (integrationProviderOptions.isManagementEnabled && query.startsWith(`/${integrationProviderOptions.command ?? 'integrations'}`)) {
  ...
  for (const integrationModule of integrationModules) {
    if (
      ...
      (integrationModule.definition.excludeFromSourceFilter || selectedSources.length === 0 || selectedSources.includes(integrationModule.definition.title))
    ) {
 ...
 */
export async function itemSelection(result: HomeDispatchedSearchResult, lastResponse?: HomeSearchListenerResponse): Promise<boolean> {
  if (result.data) {
    if (integrationHelpers?.setSearchQuery && result.action.trigger === 'user-action' && result.action.name === POPULATE_QUERY && typeof result.data?.populateQuery === 'string') {
...
export async function getManagementResults(): Promise<HomeSearchResponse> {
  ...
  if (homeResponse.results.length === 0) {
    ...
    logger.error('Integration management is enabled but you have no integrations listed in your settings or none of them are enabled');
  }
...
async function setPreference(integrationId: string, autoStart: boolean) {
  ...
  if (endpointProvider.hasEndpoint(integrationPreferenceEndpointId)) {
    const success = await endpointProvider.action<{ id: string; payload: { autoStart: boolean } }>(integrationPreferenceEndpointId, { id: integrationId, payload: { autoStart } });
    if (success) {
...
async function getPreference(integrationId: string): Promise<{ autoStart: boolean }> {
  ...
  if (endpointProvider.hasEndpoint(integrationPreferenceEndpointId)) {
    const preference = await endpointProvider.requestResponse<{ id: string }, { autoStart: boolean }>(integrationPreferenceEndpointId, { id: integrationId });
...
async function updateIntegrationStatus(lastResponse: HomeSearchListenerResponse, integrationId: string, shouldAutoStart: boolean): Promise<boolean> {
  if (!integrationProviderOptions) {
    logger.error('IntegrationProvider is not available, make sure your have called init');
  }
  ...
  const result = await createResult(integration.definition.id, integration.definition.title, integration.definition.description, integration.definition.icon, shouldAutoStart);
 ...
 */
async function createResult(id: string, name: string, description: string, icon: string, autoStart: boolean): Promise<HomeSearchResult> {
      ...
      layout: await createContainer(
        ...
        [
          await createContainer(
            'column',
            [
              await createContainer('row', [await createImage('icon', name, { width: '32px', height: '32px' }), await createTitle('title')], {
                alignItems: 'center',
                gap: '10px',
              }),
          ...
          await createContainer('column', [
            ...
            await createButton(autoStart ? ButtonStyle.Primary : ButtonStyle.Secondary, 'btnText', buttonAction),
          ]),
        ],
        {
          padding: '10px',
