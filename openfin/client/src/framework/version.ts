 ...
 */
export async function init(versionProviderOptions?: VersionProviderOptions, endpointProvider?: EndpointProvider): Promise<void> {
  if (versionProviderOptions === undefined) {
    ...
    isMonitoringEnabled =
      versionCheckIntervalInSeconds !== undefined && versionProviderOptions.endpointId !== undefined && endpointProvider.hasEndpoint(versionProviderOptions.endpointId);
...
export function setVersion(versionType: VersionType, versionNumber: string): void {
  ...
  switch (versionType) {
    case 'app': {
      logger.warn('You cannot set the version for app as this can only be specified via the settings provided on application launch inside of the version provider settings.');
      break;
    }
    case 'notificationCenter': {
      ...
      if (setVersionNumber) {
        ...
        logger.warn(`${versionLabel} already set to: ${versionInfo.notificationCenter} so we are unable to set it to ${versionNumber}`);
      }
      break;
    }
    case 'platformClient': {
      ...
      if (setVersionNumber) {
        ...
        logger.warn(`${versionLabel} already set to: ${versionInfo.platformClient} so we are unable to set it to ${versionNumber}`);
      }
      break;
    }
    case 'rvm': {
      ...
      if (setVersionNumber) {
        ...
        logger.warn(`${versionLabel} already set to: ${versionInfo.rvm} so we are unable to set it to ${versionNumber}`);
      }
      break;
    }
    case 'runtime': {
      ...
      if (setVersionNumber) {
        ...
        logger.warn(`${versionLabel} already set to: ${versionInfo.runtime} so we are unable to set it to ${versionNumber}`);
      }
      break;
    }
    case 'workspacePlatformClient': {
      ...
      if (setVersionNumber) {
        ...
        logger.warn(`${versionLabel} already set to: ${versionInfo.workspacePlatformClient} so we are unable to set it to ${versionNumber}`);
      }
      break;
    }
    case 'workspace': {
      ...
      if (setVersionNumber) {
        ...
        logger.warn(`${versionLabel} already set to: ${versionInfo.workspace} so we are unable to set it to ${versionNumber}`);
      }
      break;
    }
    case 'workspaceClient': {
      ...
      if (setVersionNumber) {
        ...
        logger.warn(`${versionLabel} already set to: ${versionInfo.workspaceClient} so we are unable to set it to ${versionNumber}`);
      }
      break;
    }
  }
...
export async function getVersionStatus(): Promise<VersionStatus> {
  ...
  if (endpointId !== undefined && endpoints !== undefined && endpoints.hasEndpoint(endpointId)) {
    try {
      const response = await endpoints.requestResponse<VersionRequest, VersionResponse>(endpointId, getVersionStatusData());
...
function validateVersionWindow(windowOptionsToValidate: OpenFin.WindowOptions): OpenFin.WindowOptions {
  ...
  if (validatedWindowOptions.url === undefined) {
    logger.error('A version window configuration was set but a url was not provided. A window cannot be launched.');
    return undefined;
  }
...
function isGreaterThanOrLessThan(versionToCheck: string, isMinLimit: boolean, type: string, baseVersion: string, limitVersion: string) {
  const maxOrMinLabel = isMinLimit ? 'minimum' : 'maximum';
  if (Number.parseInt(baseVersion, 10) > Number.parseInt(limitVersion, 10)) {
    logger.info(`Finished check for ${versionToCheck}. Current ${type} version: ${baseVersion} is greater than ${maxOrMinLabel} ${type} version: ${limitVersion}`);
    if (isMinLimit) {
      return true;
    }
    return false;
  } else if (Number.parseInt(baseVersion, 10) < Number.parseInt(limitVersion, 10)) {
    logger.info(`Finished check for ${versionToCheck}. Current ${type} version: ${baseVersion} is less than ${maxOrMinLabel} ${type} version: ${limitVersion}`);
    if (isMinLimit) {
      return false;
    }
    return true;
...
function compareVersion(versionToCheck: VersionType, isMinLimit: boolean, currentVersion: string, limitVersion: string): boolean {
  if (currentVersion === undefined || currentVersion === null || limitVersion === undefined || limitVersion === null) {
    logger.warn(`A check was requested for ${versionToCheck} but either the base version or limit version was not specified.`);
    return true;
  }
  ...
  for (let i = 0; i < currentVersion.length; i++) {
    const isValid = isGreaterThanOrLessThan(versionToCheck, isMinLimit, versionParts[i], currentVersionList[i], limitVersionList[i]);
