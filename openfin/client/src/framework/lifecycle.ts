...
import type { Lifecycle, LifecycleEvents, LifecycleHandler, LifecycleProviderOptions } from './shapes/lifecycle-shapes';
...
export async function init(lifecycleProviderOptions: LifecycleProviderOptions, helpers: ModuleHelpers): Promise<void> {
...
export async function fireLifecycleEvent(platform: WorkspacePlatformModule, lifecycleEvent: LifecycleEvents, customData?: unknown): Promise<void> {
...
export function subscribeLifecycleEvent(lifecycleEvent: LifecycleEvents, lifecycleHandler: LifecycleHandler): string {
  const id = randomUUID();
