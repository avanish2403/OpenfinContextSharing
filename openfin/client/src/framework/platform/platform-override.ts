...
import type { PageChangedLifecyclePayload, WorkspaceChangedLifecyclePayload } from 'customize-workspace/shapes';
  ...
  class Override extends WorkspacePlatformProvider {
    ...
    public async getSavedWorkspaces(query?: string): Promise<Workspace[]> {
      ...
      if (endpointProvider.hasEndpoint(getWorkspacesEndpointId)) {
        const workspacesResponse = await endpointProvider.requestResponse<{ query?: string }, { [key: string]: Workspace }>(getWorkspacesEndpointId, { query });
    ...
    public async getSavedWorkspace(id: string): Promise<Workspace> {
      ...
      if (endpointProvider.hasEndpoint(getWorkspaceEndpointId)) {
        const workspaceResponse = await endpointProvider.requestResponse<{ id: string }, Workspace>(getWorkspaceEndpointId, { id });
    ...
    public async createSavedWorkspace(req: CreateSavedWorkspaceRequest): Promise<void> {
      ...
      if (endpointProvider.hasEndpoint(setWorkspaceEndpointId)) {
        const success = await endpointProvider.action<{ id: string; payload: Workspace }>(setWorkspaceEndpointId, { id: req.workspace.workspaceId, payload: req.workspace });
        if (success) {
    ...
    public async updateSavedWorkspace(req: UpdateSavedWorkspaceRequest): Promise<void> {
      ...
      if (endpointProvider.hasEndpoint(setWorkspaceEndpointId)) {
        ...
        if (success) {
          ...
          logger.info(`Unable to update workspace with id: ${req.workspace.workspaceId} against custom storage`);
        }
        return;
      }
      logger.info(`Saving updated workspace to default storage for workspace id: ${req.workspace.workspaceId}.`);
    ...
    public async getSavedPages(query?: string): Promise<Page[]> {
      ...
      if (endpointProvider.hasEndpoint(getPagesEndpointId)) {
        const pagesResponse = await endpointProvider.requestResponse<{ query: string }, { [key: string]: Page }>(getPagesEndpointId, { query });
    ...
    public async createWindow(options: OpenFin.PlatformWindowCreationOptions, identity?: OpenFin.Identity): Promise<OpenFin.Window> {
