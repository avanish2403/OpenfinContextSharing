import { BrowserInitConfig, getCurrentSync, init as workspacePlatformInit } from '@openfin/workspace-platform';
...
async function setupPlatform(_?: PlatformProviderOptions): Promise<boolean> {
  ...
  conditionsProvider.registerCondition('authenticated', async () => isAuthenticationEnabled() && !(await isAuthenticationRequired()), false);
...
export async function init(): Promise<boolean> {
  ...
  if (!isValid) {
    logger.error('The platform cannot startup as there was a problem with the initialization of the auth flow.');
  }
  return isValid;
}
