...
export function interopOverride(
  ...
  class InteropOverride extends InteropBroker {
    public async launchAppWithIntent(app: PlatformApp, intent: OpenFin.Intent) {
      ...
      if (app.manifestType !== manifestTypes.view.id && app.manifestType !== manifestTypes.inlineView.id && app.manifestType !== manifestTypes.snapshot.id) {
    ...
    public async launchAppPicker(launchOptions: { apps?: PlatformApp[]; intent?: Partial<AppIntent>; intents?: { intent: Partial<AppIntent>; apps: PlatformApp[] }[] }): Promise<{
      appId: string;
      ...
      const url = settings?.platformProvider?.intentPicker.url || window.location.href.replace('platform/provider.html', 'common/windows/intents/picker.html');
    ...
    public async isActionAuthorized(action: string, payload: unknown, identity: OpenFin.ClientIdentity): Promise<boolean> {
    ...
    public async handleFiredIntentForContext(contextForIntent: { type: string; metadata?: { target?: string } }, clientIdentity) {
      ...
      if (contextForIntent.metadata?.target !== undefined && contextForIntent.metadata?.target !== null && contextForIntent.metadata?.target.trim().length > 0) {
      ...
      if (availableIntents.length === 1) {
        ...
        try {
          ...
          if (selectedIntent === undefined) {
            logger.error("The user selected an intent but it's name doesn't match the available intents", userSelection);
            throw new Error(NO_APPS_FOUND);
          }
    ...
    public async handleFiredIntent(intent: OpenFin.Intent) {
      ...
      if (intent.metadata?.target !== undefined && intent.metadata?.target !== null && intent.metadata?.target.trim().length > 0) {
      ...
      if (targetApp !== undefined) {
        if (!intentApps.includes(targetApp)) {
          logger.info(`Selected appId was specified: ${targetAppId} but does it does not support the intent`, intent);
          throw new Error(NO_APPS_FOUND);
        }
    ...
    public async fdc3HandleOpen(
      ...
      const requestedId = typeof fdc3OpenOptions.app === 'string' ? fdc3OpenOptions.app : fdc3OpenOptions.app.appId ?? fdc3OpenOptions.app.name;
