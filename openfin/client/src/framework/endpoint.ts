...
import type { Endpoint, EndpointDefinition, EndpointProviderOptions, FetchOptions } from './shapes/endpoint-shapes';
...
function getRequestOptions(url: string, options: FetchOptions, request: unknown): { url: string; options: FetchOptions } {
...
export async function action<T>(endpointId: string, request?: T): Promise<boolean> {
  ...
  if (endpointType === 'fetch') {
    ...
    if (req.options.method !== 'GET' && req.options.method !== 'POST') {
      logger.warn(`${endpointId} specifies a type: ${endpointType} with a method ${req.options.method} that is not supported.`);
      return false;
    }
