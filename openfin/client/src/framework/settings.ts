...
export async function getSettings(): Promise<CustomSettings> {
  if (settings === undefined) {
    ...
    if (endpointProvider.hasEndpoint('platform-settings')) {
      ...
      const serverSettings = await endpointProvider.requestResponse<unknown, CustomSettings>('platform-settings');
...
export async function isValid() {
  ...
  const possibleHosts = ['platform/provider.html', 'provider.html'];
  ...
  if (isValidHost) {
    ...
    logger.warn(`The source of the initial settings ${manifestUrl} does not match any of the valid host names. Please update the list if required.`);
  }
  return isValidHost;
}
