...
export class PagesProvider implements IntegrationModule<PagesSettings> {
   ...
   * @returns Nothing.
   */
  public async initialize(definition: ModuleDefinition<PagesSettings>, loggerCreator: LoggerCreator, helpers: IntegrationHelpers): Promise<void> {
    this._settings = definition.data;
    this._integrationHelpers = helpers;
    this._logger = loggerCreator('PagesProvider');

    this._integrationHelpers.subscribeLifecycleEvent('page-changed', async (platform: WorkspacePlatformModule, payload: PageChangedLifecyclePayload) => {
  ...
  public async getSearchResults(
    ...
    const pageResults: HomeSearchResult[] = await this.buildResults(platform, pages, queryLower, options.queryMinLength, colorScheme);
  ...
  private getPageTemplate(id: string, title: string, shareEnabled: boolean, colorScheme: ColorSchemeMode, palette: CustomPaletteSet): HomeSearchResult {
    let actions = [];
  ...
  private async buildResults(platform: WorkspacePlatformModule, pages: Page[], query: string, queryMinLength: number, colorScheme: ColorSchemeMode): Promise<HomeSearchResult[]> {
