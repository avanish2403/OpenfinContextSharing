...
export class WorkspacesProvider implements IntegrationModule<WorkspacesSettings> {
   ...
   * @returns Nothing.
   */
  public async initialize(definition: ModuleDefinition<WorkspacesSettings>, loggerCreator: LoggerCreator, helpers: IntegrationHelpers): Promise<void> {
    this._settings = definition.data;
    this._integrationHelpers = helpers;
    this._logger = loggerCreator('WorkspacesProvider');

    this._integrationHelpers.subscribeLifecycleEvent('workspace-changed', async (platform: WorkspacePlatformModule, payload: WorkspaceChangedLifecyclePayload) => {
  ...
  public async getSearchResults(
    ...
    const workspaceResults: HomeSearchResult[] = await this.buildResults(platform, workspaces, queryLower, options.queryMinLength, colorScheme);
  ...
  private getWorkspaceTemplate(id: string, title: string, shareEnabled: boolean, isCurrent: boolean, colorScheme: ColorSchemeMode, palette: CustomPaletteSet): HomeSearchResult {
    let actions = [];
    let layout;
    let data;
  ...
  private getOtherWorkspaceTemplate(enableShare: boolean, enableDelete: boolean, palette: CustomPaletteSet): TemplateFragment {
  ...
  private async buildResults(
    ...
    if (Array.isArray(workspaces)) {
      ...
      results = workspaces
        ...
        .map((ws: Workspace, index: number) => this.getWorkspaceTemplate(ws.workspaceId, ws.title, shareEnabled, currentWorkspaceId === ws.workspaceId, colorScheme, palette))
