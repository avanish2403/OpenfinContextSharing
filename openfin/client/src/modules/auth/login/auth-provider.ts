...
export class ExampleAuthProvider implements AuthProvider<ExampleOptions> {
   ...
   * @returns Nothing.
   */
  public async initialize(definition: ModuleDefinition<ExampleOptions>, createLogger: LoggerCreator, helpers: ModuleHelpers) {
   ...
   */
  public subscribe(to: 'logged-in' | 'before-logged-out' | 'logged-out' | 'session-expired', callback: () => Promise<void>): string {
    ...
    if (this._authOptions?.checkSessionValidityInSeconds !== undefined && this._authOptions?.checkSessionValidityInSeconds > -1 && this._sessionExpiryCheckId === undefined) {
      this._sessionExpiryCheckId = window.setTimeout(async () => {
        ...
        if (stillAuthenticated) {
          ...
          this._logger.info(
            'Session not valid. Killing session and notifying registered callback that authentication is required. This check is configured in the data for this auth module. Set checkSessionValidityInSeconds to -1 in the authProvider module definition if you wish to disable this check',
          );
          this._authenticated = false;
          clearCurrentUser();
