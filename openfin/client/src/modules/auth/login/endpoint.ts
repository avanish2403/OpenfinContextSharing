...
export class ExampleAuthEndpoint implements Endpoint<ExampleEndpointOptions> {
   ...
   * @returns Nothing.
   */
  public async initialize(definition: ModuleDefinition<ExampleEndpointOptions>, createLogger: LoggerCreator, helpers?: ModuleHelpers) {
   ...
   */
  public async requestResponse(endpointDefinition: EndpointDefinition<FetchOptions>, request?: unknown): Promise<unknown> {
  ...
  private getRequestOptions(url: string, options: FetchOptions, request: unknown): { url: string; options: FetchOptions } {
